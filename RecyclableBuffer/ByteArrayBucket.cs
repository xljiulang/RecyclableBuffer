...
namespace RecyclableBuffer
{
    ...
    public abstract class ByteArrayBucket
    {
        ...
        /// </summary>
        /// <param name="arrayLength">每个数组的长度。</param>
        /// <param name="maxArrayCount">最大数组数量。</param>
        /// <returns>固定大小字节数组桶实例。</returns>
        public static ByteArrayBucket CreateFixedSize(int arrayLength, int maxArrayCount)
        {
            return new FixedSizeByteArrayBucket(arrayLength, maxArrayCount);
        }
        ...
        private sealed class FixedSizeByteArrayBucket : ByteArrayBucket
        {
            ...
            /// <param name="arrayLength">每个数组的长度。</param>
            /// <param name="maxArrayCount">最大数组数量。</param>
            public FixedSizeByteArrayBucket(int arrayLength, int maxArrayCount)
            {
                this._arrayLength = GetMaxArrayLength(arrayLength);
                this._pool = ArrayPool<byte>.Create(arrayLength, maxArrayCount);
            }

            /// <inheritdoc/>
            public override byte[] Rent()
            {
                return _pool.Rent(this._arrayLength);
            }
